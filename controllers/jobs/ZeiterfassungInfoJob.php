...
class ZeiterfassungInfoJob extends JOB_Controller
{
	...
	public function sendMails()
	{
		...
		if ($this->sendAllMail())
		{
			$this->logInfo('SUCCEDED: Sending E-Mails regarding Timesheets succeded.');
		}
		else
		{
			$this->logError('Error when sending emails in job ZeiterfassungInfoJob in FHC-Core-Casetime Extension');
		}
	}

	public function sendAllMail()
	{
		$allMitarbeiter = $this->_getEmployeeUids();
		...
		$mitarbeiter_timesheets = $this->_getAllTimesheets();
		$mailingList = array();
		...
		foreach ($allMitarbeiter as $ma)
		{
			...
			if(!in_array($uid, $mitarbeiter_timesheets))
			{
				$ma->EmpZeitMod = true;
			}
			else
	...
	private function _getEmployeeUids()
	{
	...
	private function _getAllTimesheets()
	{
		$mitarbeiterList = $this->TimesheetModel->getAllMissingZeitmodelle();
		$names = array();
		if(!empty($mitarbeiterList))
		{
			foreach ($mitarbeiterList[1] as $ma)
			{
				if(!empty($ma))
				{
					array_push($names, strtolower($ma[0]));
				}
			}
		}
